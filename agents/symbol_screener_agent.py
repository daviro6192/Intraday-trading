...
ciclo lento) per scegliere i 3 simboli su cui tradare tra un universo di
...
scambiato in USDT nelle ultime 24h.

Il tuo compito è scegliere ESATTAMENTE 3 simboli tra quelli candidati su
cui tradare per l'intera sessione odierna:
...
Rispondi esclusivamente tramite il tool fornito, con esattamente 3 simboli
scelti tra quelli dell'elenco candidato (usa lo stesso ticker perpetual
esatto ricevuto in input, es. "BTCUSDT").
"""
...
class SymbolScreenerAgent(Agent):
    ...
    def run(self, candidates: list[SymbolCandidate]) -> SymbolSelection:
        user_message = (
            "Candidati (statistiche 24h da Binance futures):\n"
            f"{json_dumps_compact([c.model_dump(mode='json') for c in candidates])}\n\n"
            "Scegli esattamente 3 simboli tra questi per la sessione di trading di oggi."
        )
